function NearestFC = import_OBS_NearestFC(filename)
%IMPORT_OBS_NEARESTFC Import the observations and the nearest forecasts
%from csv files.
%  NEARESTFC = IMPORT_OBS_NEARESTFC(FILENAME) reads observations and the 
%  correspondent nearest forecasts from csv file FILENAME.
%  Note: the first column correspond to the observation. the rest columns
%  correspond to the correspondent nearest forecasts.
%
%  Example:
%  NearestFC = importfile("/perm/mo/mofp/PhD/Papers2Write/OperUse_ecPointRainfall/Pre_Process/Processed_Data/OBS/OMSZ/ObjectiveVerif/NearestFC_ECMWFObs/NearestFC_012_20200601_00_012.csv");
...
% Import the data
NearestFC = readtable(filename, opts);

%% Convert to output type
NearestFC = table2array(NearestFC);
end