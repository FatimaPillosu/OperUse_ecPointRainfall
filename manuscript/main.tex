...
\begin{figure}
\centerline{\includegraphics[width=19pc]{manuscript/Figures/UND_EFF_TradEPFs.png}}
\caption{Relationship between users' understanding (UND) of ensemble-based probabilistic forecasts and their effective (EFF) use in decision-making. Diagram's x-axis represents UND levels: "low", "medium", "high". Over time, training on ensemble-based probabilistic forecasts (cyan arrow in the x-axis direction) can push UND from "low" to "medium" or "high" levels. Diagram's y-axis represents EFF levels: "ineffective", "moderately effective", "very effective". Over time, training on how to communicate ensemble-based probabilistic outputs (cyan arrow in the y-axis direction) can push their effectiveness in decision-making from "ineffective" to "moderately effective" or "very effective". The green line represents optimal values for the UND-EFF relationship, and no UND-EFF pairs can exist in the green dashed area above the green line. Qualitatively, the blue line represents state-of-the-art UND-EFF relationship for traditional probabilistic ensemble-based forecasts.}
\label{UND_EFF_TradEPFs}
\end{figure}
