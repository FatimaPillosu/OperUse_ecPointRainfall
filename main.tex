...
% TITLE %
%%%%%%%%%
\title{Improving the communication of new point rainfall forecasts through a dialogue with forecasters}
...
% ABSTRACT %
%%%%%%%%%%%%
\abstract{In April 2019, new statistically post-processed point-rainfall forecasts (ecPoint-Rainfall) became operational at the European Centre for Medium-Range Weather Forecasts. An extensive verification study shows that ecPoint-Rainfall has higher reliability/skill than raw ECMWF ensemble for forecasting rainfall at a point. However, ecPoint-Rainfall adds layers of complexity to standard probabilistic ensemble-based forecasts (PEFs) and, before this study, it was not clear to what extent forecasters might find its interpretation conceptually challenging, even for those with an extensive probabilistic forecasting experience. This study was therefore deemed necessary to evaluate whether traditional guidelines for PEFs would be sufficient for the adoption of ecPoint-Rainfall, or whether they would need to be somehow tailored to take account of the specific features of this new product to best communicate and exploit its added value. During a one-year trial period, ecPoint-Rainfall was tested in an operational environment at two meteorological centres with different expertise on PEFs: the Hungarian Meteorological Service and the National Meteorological Institute of Costa Rica. Later, crucial aspects that conditioned or might condition in future the adoption of ecPoint-Rainfall were explored with the forecasters that used ecPoint-Rainfall in their day-to-day work. This paper summarises the outcomes of the initial independent feedback and the subsequent discussions on how valuable ecPoint-Rainfall was perceived. The overall impression is that ecPoint-Rainfall could improve the prediction of extreme localized rainfall events only if tailored training is provided, no matter what is the prior forecasters experience on PEFs.}
...
\textit{"Forecasts possess no intrinsic value. They acquire value through their ability to influence the decisions made by users of the forecasts”} \citep{Murphy1993}. With this sentence, Murphy states that increases in forecast accuracy do not necessarily imply increases in forecast value as the quality/value relationship is user and problem-specific. \par
...
\begin{figure}
\centerline{\includegraphics[width=19pc]{Figures/Fig1.jpg}}
...
IMN, in Costa Rica (see Fig.\ref{Fig1}a), has a recent experience with EPSs and with ECMWF forecasts, which started using in 2018. They work mainly with the Global Forecast System (GFS) developed by the National Centers for Environmental Prediction in USA and with the Weather Research and Forecasting (WRF) system. From the latter, IMN has developed a series of model versions (WRF-1.5, WRF-5, WRF-AR, WRF-Sarapiquí, WRF, WRF-8, WRF-15) that are tailored to different needs, such as prediction of tropical waves, tropical cyclones, cold fronts, hail and forest fires. Such versions have different spatial and temporal resolutions, different domains, and different model configurations. The most recent model, WRF-1.5, was developed in 2018 at IMN to produce forecasts at 1.5 km resolution up to 5 days in advance to improve the predictions for convective systems that can generate extreme rainfall events in Costa Rica. 
}

{\color{red}
\subsection{Hungarian Meteorological Service (OMSZ)}
OMSZ, in Hungary (see Fig.\ref{Fig1}b), has a long experience working with EPSs. They are at the forefront of developing ensemble based products, adopted also by other centres, see for example \citet{Gascon2018}, and they use probabilistic forecasts in their day-to-day operational duties to use warning to the public and end-users. OMSZ has a long history of collaborations with ECMWF. Not only OMSZ has a long tradition of carrying out objective verification of ECMWF ensemble forecasts \citep{Hewson2020}, but it also uses widely ECMWF’s software and archived forecasts for all time ranges since 1995. Forecasters from OMSZ also take part regularly in educational and training programmes at ECMWF. \par
...
\begin{figure*}
\centerline{\includegraphics[width=39pc]{Capture.png}}
...
ecPoint is a statistical post-processing technique that provides probabilistic bias-corrected forecasts at point scale from ensemble or deterministic numerical weather prediction (NWP) model outputs. Different hydro-meteorological variables, from different NWP models, can potentially be post-processed using the ecPoint technique, generating the so-called "ecPoint Family". ecPoint-Rainfall is the branch of the family that produces rainfall forecasts that mirror rain gauge measurements \citep{Hewson2020a}. In this paper, the ecPoint-Rainfall forecasts post-process raw ECMWF ENS rainfall forecasts. It is worth noting that what described to the following also applies to other NWP models. \par
...
\citet{Hewson2020a} have shown that, at every lead time, up to 10 days, ecPoint-Rainfall outperforms raw ECMWF ENS when predicting localise rainfall events, including also those extreme. \par
...
According to \citet{Fundel2019}, users need the opportunity to familiarise with the quality of new products in their everyday practice in order to appreciate their entire value in decision-making processes. OMSZ and IMN volunteered to test the ecPoint-Rainfall forecasts in their day-to-day operational routines. The participation of OMSZ and IMN was ideal as the two meteorological services have different degrees of experience with probabilistic forecasts, and reside in two different climatological regions, tropical for IMN and extra-tropical for OMSZ, so that allows the evaluation of ecPoint-Rainfall in different climates. The trial period started in 2018. During the trial, OMSZ and IMN were invited to analyse the ecPoint-Rainfall performance in predicting extreme rainfall events, and finally summarise their results in a report to send to ECMWF at the end of the trial. Given the small volumes of data available, both countries decided to carry out their analysis on independently selected case studies. The ecPoint-Rainfall forecasts were presented to both countries at the beginning of the trial. They were informed of different sources of information about ecPoint-Rainfall on the web, such as newsletters or the ECMWF Forecaster's User Guide \citep{Owens2018} and no further details were provided unless direct requests were submitted.
	
\subsection{Dialogue forecasters-developers}
The forecasters who monitored the ecPoint-Rainfall forecasts in their day-to-day operational duties were invited to discuss the outcomes of the reports. Forecasters were also invited to discuss the theoretical and practical challenges that emerged when adopting ecPoint-Rainfall in an operational environment. Such an open-ended approach are particularly well suited to explore behavioural responses to innovative forecast products like ecPoint-Rainfall, where the range of possible conceptual framings and the meaningful behaviour of the informants is not quantifiable but must be teased out through careful interpretation \citep{Patton2002}.	
...
\begin{figure}
\centerline{\includegraphics[width=18pc]{Fig2.jpg}}
\caption{Product developed by IMN based on the 85th percentile of ecPoint-Rainfall.}
\label{Fig2}
\end{figure}

\begin{figure*}
\centerline{\includegraphics[width=39pc]{Fig3.jpg}}
\caption{Panel (a) shows the climate regions of Costa Rica defined by IMN. Panels from (o-1) to (o-6) show 12-hourly rainfall observations between October 3rd and 5th, 2018. Panels from (i-1) to (i-3) show the impacts caused by the extreme event in the regions in which CNE declared the state of red alert (the photos come from www.teletica.com).}
\label{Fig3}
\end{figure*}
...
\begin{figure}
\centerline{\includegraphics[width=19pc]{Fig4.jpg}}
...
%%%%%%%%%%%%%%%%%%%
\acknowledgments
We thank the anonymous reviewers, Florian Pappenberger, David Richardson and Christel Prudhomme who helped to improve the manuscript with their comments.
